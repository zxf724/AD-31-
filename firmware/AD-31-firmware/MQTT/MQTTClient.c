...
#include "datasave.h"
#include "time.h"
void  databasepack(char *cmd, cJSON *desired, char *buf){
    char *s = NULL;
    cJSON *root = NULL;
    ...
    if (root != NULL) {
        ...
        cJSON_AddNumberToObject(root, "timestamp", time((time_t *)NULL));
...
static int sendPacket(MQTTClient *c, int length, Timer *timer)
{
    int rc = FAILURE,
        sent = 0;
    DBG_LOG("c->buf:%*s,length:%d",length, c->buf ,length);
...
static int readPacket(MQTTClient *c, Timer *timer){
    ...
    if (c->ipstack->mqttread(c->ipstack, c->readbuf, 1, TimerLeftMS(timer)) != 1) goto exit;
    ...
    if (rem_len > 0 && (c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len, TimerLeftMS(timer)) != rem_len)) goto exit;
...
exit:
    ...
    int len = 0;
    //int rem_len = 0;
    len = c->ipstack->mqttread(c->ipstack, c->readbuf, SOCKET_FIFO_SIZE, TimerLeftMS(timer)); //
    c->readbuf_size = len;
    if(len > 0){
...
int deliverMessage(MQTTClient *c, MQTTString *topicName, MQTTMessage *message)
{
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i) {
        if (c->messageHandlers[i].fp != NULL) {
            MessageData md;
            NewMessageData(&md, topicName, message);
            //DBG_LOG("NewMessageData :%s",md.message->payload);
            c->messageHandlers[i].fp(&md);
            rc = SUCESS;
        }
    }
...
int keepalive(MQTTClient *c)
{
    ...
    if (TimerIsExpired(&c->ping_timer)) {
        if (!c->ping_outstanding) {
            ...
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == SUCESS) // send the ping packet
...
exit:
    ...
    if (TimerIsExpired(&c->ping_timer)){
        if (!c->ping_outstanding) {
            ...
            TimerInit(&timer);
            TimerCountdownMS(&timer, 1000);
            //Publish_ConnectHeart("");
            //int len = MQTTSerialize_pingreq(c->buf, c->buf_size);
            int len = heart((char*)c->buf);
            if (len > 0 && (rc = sendPacket(c, len, &timer)) == SUCESS) // send the ping packet
                c->ping_outstanding = 1;
        }
    }
...
//extern void ArrivePath(uint8_t *dat, uint16_t len);
    ...
    case PUBLISH:
        {
            ...
            if (MQTTDeserialize_publish(&msg.dup, &intQoS, &msg.retained, &msg.id, &topicName,
                                        (unsigned char **)&msg.payload, (int *)&msg.payloadlen, c->readbuf, c->readbuf_size) != 1) goto exit;
            msg.qos = (enum QoS)intQoS;
            deliverMessage(c, &topicName, &msg);
            if (msg.qos != QOS0) {
                if (msg.qos == QOS1) len = MQTTSerialize_ack(c->buf, c->buf_size, PUBACK, 0, msg.id);
                else if (msg.qos == QOS2) len = MQTTSerialize_ack(c->buf, c->buf_size, PUBREC, 0, msg.id);
                if (len <= 0) {
                    rc = FAILURE;
                } else {
    ...
    case PUBREC:
        {
            ...
            else if ((len = MQTTSerialize_ack(c->buf, c->buf_size, PUBREL, 0, mypacketid)) <= 0) rc = FAILURE;
            else if ((rc = sendPacket(c, len, timer)) != SUCESS) // send the PUBREL packet
...
void MQTTRun(void *parm)
{
    ...
    TWDT_DEF(mqttTask, 60000);
    TWDT_ADD(mqttTask);
    TWDT_CLEAR(mqttTask);

    TimerInit(&timer);

    DBG_LOG("MQTT task start.");
    while (1) {
#if defined(MQTT_TASK)
        MutexLock(&c->mutex);
#endif
...
int MQTTConnect(MQTTClient *c, MQTTPacket_connectData *options){
    ...
    if ((rc = sendPacket(c, len, &connect_timer)) != SUCESS)  // send the connect packet//
        goto exit; // there was a problem
    if (waitfor(c, CONNACK, &connect_timer) == CONNACK) {
        unsigned char connack_rc = 255;
        unsigned char sessionPresent = 0;

        if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, c->readbuf, c->readbuf_size) == 1) rc = connack_rc;
        else rc = FAILURE;
    } else rc = FAILURE;

exit:
    if (rc == SUCESS) c->isconnected = 1;
    ...
    c->keepAliveInterval = options->keepAliveInterval;
    TimerCountdown(&c->ping_timer, c->keepAliveInterval);
    len = log_in((char*)c->buf);
    //if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0) goto exit;
    if ((rc = sendPacket(c, len, &connect_timer)) != SUCESS || len == 0)  // send the connect packet//
        goto exit; // there was a problem
...
int MQTTUnsubscribe(MQTTClient *c, const char *topicFilter){
    ...
    if ((rc = sendPacket(c, len, &timer)) != SUCESS) // send the subscribe packet
        goto exit; // there was a problem
...
exit:
    ...
    memcpy(c->buf, message->payload, message->payloadlen);
    len = message->payloadlen;
    //DBG_LOG("c->buf:%*s,payloadlen:%d",len,c->buf ,len);
    //DBG_LOG("messag  e->payload:%s,payloadlen:%d",c->buf ,message->payloadlen);
    if (len <= 0) goto exit;
    if ((rc = sendPacket(c, len, &timer)) != SUCESS) // send the subscribe packet
        goto exit; // there was a problem
    exit:
...
int MQTTDisconnect(MQTTClient *c)
{
    ...
    if (len > 0) rc = sendPacket(c, len, &timer);            // send the disconnect packet

...
int16_t System_SockSend(uint8_t *data, uint16_t len)
{
...
#if NET_M2M_EN > 0
    if (sent == NET_M2M) {
        sent = M2M_SocketSendData(data, len);
    }
#endif
    ...
    if (sent == NET_WIFI) {
        ...
        //DBG_LOG("WIFI_SocketSendData:%d", sent);
    }
#endif
...
#if NET_M2M_EN > 0
    M2M_SetSocketParam(socketPar.addr, socketPar.port, SocketRec);
#endif
...
int8_t System_SockConnect(char *addr, uint16_t port)
{
    ...
    M2M_SetSocketParam(addr, port, SocketRec);
#endif
...
int8_t System_SockIsConnected(char **addr, uint16_t *port)
{
    ...
    /*网络使用的优先级，LAN > WIFI > M2M*/
#if NET_M2M_EN > 0
    if (M2M_IsSocketConnect() > 0) {
        ret = NET_M2M;
    }
#endif
#if NET_WIFI_EN > 0
    if (WIFI_IsSocketConnect() > 0) {
        ret = NET_WIFI;
    }
#endif
#if NET_LAN_EN > 0
...
#if NET_M2M_EN > 0
            if (ret != NET_M2M) {
                M2M_SetSocketParam(NULL, 0, NULL);
            }
#endif
        }
    } else {
...
#if NET_M2M_EN > 0
    M2M_Status_t statusM2M;
    statusM2M = M2M_ReadStatus();
#endif
#if NET_WIFI_EN > 0
    WIFI_Status_t statusWIFI;
    statusWIFI = WIFI_ReadStatus();
#endif
#if NET_LAN_EN > 0
...
#if NET_WIFI_EN > 0
        else if (statusWIFI == wifi_status_elink) {
            err = 8;
        } 
#endif
#if NET_M2M_EN > 0
        else if (statusM2M == M2M_status_nonet) {
            err = 3;
        } 
        
#endif
#if NET_LAN_EN > 0 && NET_WIFI_EN > 0
        else if (statusWIFI == wifi_status_station_notconnected && statusLAN == lan_status_phy_offline) {
            err = 2;
        } 
#elif NET_WIFI_EN && NET_WIFI_EN > 0
        else if (statusWIFI == wifi_status_station_notconnected) {
            err = 2;
        }
#elif NET_LAN_EN > 0
        else if (statusLAN == lan_status_phy_offline) {
            err = 2;
        }
#endif
#if NET_M2M_EN > 0
        else if (statusM2M == M2M_status_nocard) {
            err = 4;
        }
#endif
        else {
...
static void system_Console(int argc, char *argv[])
{
    ...
    if (ARGV_EQUAL("reset")) {
        ...
        DBG_LOG("Firmware version:%s, Hardware version:%s",
        VERSION,
        VERSION_HARDWARE);

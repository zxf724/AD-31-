...
char DeQueue(CirQueue_TypeDef *queue, char *buff, uint16_t len);
...
void ByteToHexStr(const unsigned char* source, char* dest, uint16_t sourceLen);
void Hex2Str( const char *sSrc,  char *sDest, uint16_t nSrcLen );
void HexStrToByte(const char* source, unsigned char* dest, uint16_t sourceLen);
#endif
